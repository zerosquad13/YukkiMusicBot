...
async def start_stream_video(message, file, title, mystic):
    ...
    if await is_active_chat(message.chat.id):
        ...
        await mystic.delete()
        cap = f"🎬 <b>Video:</b> [Given Video Via Telegram]({link})\n🎧 **Requested by:** {message.from_user.mention}"
        final_output = await message.reply_photo(
            photo="Utils/Telegram.JPEG",
...
async def start_video_stream(
    ...
    if await is_active_chat(CallbackQuery.message.chat.id):
        ...
        final_output = await CallbackQuery.message.reply_photo(
            photo=thumb,
            caption=(
                f"🏷 <b>Name:</b> [{title[:25]}](https://t.me/{BOT_USERNAME}?start=info_{videoid}) \n⏱ <b>Duration:</b> {duration_min} \n<code>Video Queued at #{position}</code>\n🎧 <b>Requested by:</b> {CallbackQuery.from_user.mention}"
            ),
            reply_markup=InlineKeyboardMarkup(buttons),
        )
        await CallbackQuery.message.delete()
